...
function CodeBlock(block)
  ...
  if genurl and img_converter == plantuml then
    ...
    print("Writing block text to temp file: " .. temp_file)
    local file = io.open(temp_file, "w")
    file:write(block.text)
    file:close()

    local args = {"-jar", plantuml_path, "-encodeurl", temp_file}
    local text = ""
    
    local ok, encoded_source = pcall(pandoc.pipe, java_path, args, text)

    if not encoded_source then
      error('Error running plantuml: no output')
    end

    -- Create the URL
    generated_url = "http://www.plantuml.com/plantuml/png/" .. encoded_source
